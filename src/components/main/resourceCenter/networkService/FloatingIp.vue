<template>
    <div class="operation-main-content">
        ...
        <NavTitle :titles="['运维中心', '资源中心', '计算服务', '公网IP']"></NavTitle>
...
export default {
        ...
        return {
            ...
            resourceInfo: {
                ...
                status: {
                    label: '公网IP状态',
                    value: '',
                    lists: this.$constant.FLOATINGIP_STATUS
                }
            },
            itemData:[
                {
                    prop: "bottomFloatingIpId",
                    label: "资源ID",
                    width: '260'
                },
                {
    ...
    methods:{
        getDataList(val){
            this.loading = true
            this.axios.get(sessionStorage.getItem(this.$constant.APP_KEY.console)+'/vFloatingIp/page',{
                params:{
                    page: val,
                    rows: this.$refs.baseTableData.pageSize,
                    name: this.resourceInfo.name.value,
                    status: this.resourceInfo.status.value,
                    bottomFloatingIpId: this.resourceInfo.resourceId.value
                }})
                .then((response) => {
                    this.loading = false
                    ...
                    if(response.data.code === this.$constant.SUCCESS_CODE){
                        ...
                        for(let data of Object.values(this.tableData)) {
                            data['className'] = this.$getStatusStyle('FLOATINGIP_STATUS',data.status)
                            data.status = this.$conversionType('floatingIp_status', data.status)
                            data.externalIp = data.externalIp ? data.externalIp : '--'
                        }
                    }else{
        ...
        onExport(bottomId,name,status){
            ...
            window.location.href = `${consoleUrl}/vFloatingIp/export?bottomFloatingIpId=${bottomId}&name=${name}&status=${status}`
        },
        tableRefresh(val){
        ...
        gotoResourceDetailPage(type, data){
            debugger
            this.$router.push({
                path: `${this.$route.matched[0].path}/floatingIpDetail`,
                query:{
                    platform: type,
                    floatingIpId: data.floatingIpId
                }
            })
        }
    }
