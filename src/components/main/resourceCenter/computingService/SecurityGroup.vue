<template>
    <div class="operation-main-content">
        ...
        <NavTitle :titles="['运维中心', '资源中心', '计算服务', '安全组']"></NavTitle>
...
export default {
    ...
    methods:{
        getDataList(val){
            this.loading = true
            this.axios.get(sessionStorage.getItem(this.$constant.APP_KEY.console)+'/vSecurityGroup/page',{params:{
                    page: val,
                    rows: this.$refs.baseTableData.pageSize,
                    name: this.resourceInfo.name.value,
                    status: this.resourceInfo.status.value,
                    bottomSecurityGroupId: this.resourceInfo.resourceId.value
                }})
                .then((response) => {
                    this.loading = false
                    ...
                    if(response.data.code === this.$constant.SUCCESS_CODE){
                        ...
                        for(let data of Object.values(this.tableData)) {
                            ...
                            data.status = this.$conversionType(this.$constant.FIREWALL_STATUS, data.status)
                        }
                    }else{
        ...
        onExport(bottomId,name,status){
            ...
            window.location.href = `${consoleUrl}/vSecurityGroup/export?bottomSecurityGroupId=${bottomId}&name=${name}&status=${status}`
        },
        tableRefresh(val){
        ...
        gotoResourceDetailPage(type, data){
            this.$router.push({
                path: `${this.$route.matched[0].path}/securityGroupDetail`,
                query:{
                    platform: type,
